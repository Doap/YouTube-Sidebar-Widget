	...
	*/
	...
	class YoutubeSidebarWidget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			$height = ($width * 3) / 4;
			$username_url = "http://gdata.youtube.com/feeds/base/users/$ident/uploads?orderby=published";
			$playlist_url = "http://gdata.youtube.com/feeds/api/playlists/$ident";
			echo $before_widget;
			if($title) {
			...
			if($ident_type == "username") {
				...
				if($youtube_is_up) {
					...
					foreach($simple_xml->channel->item as $item){
						if($fil) {
							if(strpos($item->description, $fil)) {
								if(count($list_items) < $num) {
									$link = explode("/watch?v=", $item->link);
									$hash = explode("&", $link[1]);
									$img = "http://i2.ytimg.com/vi/$hash[0]/0.jpg";

									if($titles == "yes") {
										$list_items[] = "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r<a href='$item->link'>$item->title</a>";
									} else {
										$list_items[] = "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r";
									}
					...
					if($fil) : foreach($list_items as $li) : $output .= $li; endforeach; endif;
					$output .= "</ul>\n\r";
				} else {
